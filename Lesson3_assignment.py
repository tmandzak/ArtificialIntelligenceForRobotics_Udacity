...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        alfa = motion[0] + random.gauss(motion[0], self.steering_noise)
        d = motion[1] + random.gauss(motion[1], self.distance_noise)
        
        if abs(alfa) < 0.001:
            orientation1 = self.orientation
            x1 = self.x + d*cos(orientation1)
            y1 = self.y + d*sin(orientation1)
        else:    
            R = self.length / tan(alfa)
            beta = d / R
